...
$html .= '<th width="1%"></th>';
$html .= '<th width="23%">ยศ นาม  นามสกุล</th>';
$html .= '<th width="11%">หมายเลขประจำตัว</th>';
$html .= '<th width="30%">ตำแหน่ง</th>';
$html .= '<th width="24%" colspan="8" align="center">เครื่องราชอิสริยาภรณ์/ปีที่รับ</th>';
$html .= '<th width="12%">หมายเหตุ</th>';
$html .= '</tr>';

$html .= '<tr>';
$html .= '<th colspan="5"></th>';
$html .= '<th width="3%">บ.ม.</th>';
$html .= '<th width="3%">บ.ช.</th>';
$html .= '<th width="3%">จ.ม.</th>';
$html .= '<th width="3%">จ.ช.</th>';
$html .= '<th width="3%">ต.ม.</th>';
$html .= '<th width="3%">ต.ช.</th>';
$html .= '<th width="3%">ท.ม.</th>';
$html .= '<th width="3%">ท.ช.</th>';
$html .= '<th width="10%"></th>';
$html .= '</tr>';
$html .= '</thead>';
$html .= '<tbody>';
foreach ($persons as $person) {
    ...
    $tm = $this->person_data->cdec_year2($person['BIOG_ID'], array('ตม.', 'ต.ม.'));
    $tc = $this->person_data->cdec_year2($person['BIOG_ID'], array('ตช.', 'ต.ช.'));
    $thm = $this->person_data->cdec_year2($person['BIOG_ID'], array('ทม.', 'ท.ม.'));
    $thc = $this->person_data->cdec_year2($person['BIOG_ID'], array('ทช.', 'ท.ช.'));

    $html .= '<tr nobr="true">';
    $html .= '<td width="3%">' . $num . '</td>';
    $html .= '<td width="1%">' . $n5 . '</td>';
    $html .= '<td width="23%">' . $person['BIOG_NAME'] . '</td>';
    $html .= '<td width="11%">' . $person['BIOG_ID'] . '</td>';
    $html .= '<td width="30%">' . $person['BIOG_POSNAME'] . '</td>';
    $html .= '<td width="3%">' . $bm . '</td>';
    $html .= '<td width="3%">' . $bc . '</td>';
    $html .= '<td width="3%">' . $jm . '</td>';
    $html .= '<td width="3%">' . $jc . '</td>';
    $html .= '<td width="3%">' . $tm . '</td>';
    $html .= '<td width="3%">' . $tc . '</td>';
    $html .= '<td width="3%">' . $thm . '</td>';
    $html .= '<td width="3%">' . $thc . '</td>';
    $html .= '<td width="8%">' . '&nbsp;' . '</td>';
    $html .= '</tr>';
